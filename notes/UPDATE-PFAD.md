...
- **Deploy-Domain ändern**: EINMAL in `site/astro.config.mjs` (`site:`) — canonical, og:image-URL und sitemap ziehen automatisch mit. Danach im og-Template (`site/og-image/template.html`) die URL-Zeile anpassen und og neu rendern. Domain seit 06.09.2026: **info-afd.de** (eigene Domain, Pages serviert vom Branch-Root → `base: '/'`, KEIN `/fck_afd/`-Präfix mehr). Die `CNAME` liegt in `site/public/` und muss bei jedem Deploy mit im dist landen — fehlt sie, verliert die Domain ihre Verknüpfung. **Rollback-Anker-Buchhaltung:** Jeder Deploy verschiebt `origin/gh-pages` — vor jedem Deploy den aktuellen Stand notieren, DAS ist der Rollback-Ziel-Wert. (Buchung: v1.5 = `8b9f1ab`, Live-v1.6-Deploy = `d0d02da`, Hero-Pin-Fix 13.09. = `a09d84b`, Wheel-Race-Fix 13.09. = `683642e`, Cleanup-Root-Deploy 13.09. = `5d68b47`, Preview-Runden 1–5 13.09. = `6eca870` → `a6b9cdb` → `5df584f` → `48c6d45` → `ee17899` → `85fef21` → `22a6d4e`, Schärfung-Root-Deploy 13.09. = `22a6d4e`, Umami-Tracker-Root-Deploy 13.09. = `3259a5f`, Tour-Tracking 13.09. = `0256d94` → `fcdcc38` (FALSCH, Subpfad im Root) → `b996ed7` (Fix), ChapterBreak-Summary-oben 13.09. = `b996ed7` → Luft+Grau `4930128`, finale Graustufe 70% `25bb96d`, Tracking-Erweiterung (quiz/share/vitals) `dba3fef` — jeweils Vorstand notieren, nicht den eigenen Deploy!)
