...
- **Deploy-Domain ändern**: EINMAL in `site/astro.config.mjs` (`site:`) — canonical, og:image-URL und sitemap ziehen automatisch mit. Danach im og-Template (`site/og-image/template.html`) die URL-Zeile anpassen und og neu rendern. Domain seit 06.09.2026: **info-afd.de** (eigene Domain, Pages serviert vom Branch-Root → `base: '/'`, KEIN `/fck_afd/`-Präfix mehr). Die `CNAME` liegt in `site/public/` und muss bei jedem Deploy mit im dist landen — fehlt sie, verliert die Domain ihre Verknüpfung. **Rollback-Anker-Buchhaltung:** Jeder Deploy verschiebt `origin/gh-pages` — vor jedem Deploy den aktuellen Stand notieren, DAS ist der Rollback-Ziel-Wert. (Buchung: v1.5 = `8b9f1ab`, Live-v1.6-Deploy = `d0d02da`, Hero-Pin-Fix 13.09. = `a09d84b`, Wheel-Race-Fix 13.09. = `683642e`, Cleanup-Root-Deploy 13.09. = `5d68b47`, Preview-Runden 1–5 13.09. = `6eca870` → `a6b9cdb` → `5df584f` → `48c6d45` → `ee17899` → `85fef21` → `22a6d4e`, Schärfung-Root-Deploy 13.09. = `22a6d4e`, Umami-Tracker-Root-Deploy 13.09. = `3259a5f`, Tour-Tracking 13.09. = `0256d94` → `fcdcc38` (FALSCH, Subpfad im Root) → `b996ed7` (Fix), ChapterBreak-Summary-oben 13.09. = `b996ed7` → Luft+Grau `4930128`, finale Graustufe 70% `25bb96d`, Tracking-Erweiterung (quiz/share/vitals) `dba3fef`, ref-Kanal-Attribution `4e7da11`, Szenen-Slugs (Anker + /szene/<slug> + Umami-Slugs) `c8245cc`, Howto-Szene `d209d92` → Preview-Fix `79173f7` — jeweils Vorstand notieren, nicht den eigenen Deploy!)
- 13.09.2026: Analytics-Wechsel Umami → Matomo (CT 109, matomo.nwk-pro.dev). Lokale Commits: 1a1b8d0 (Umami-Hash-Pageview-Fix, nie deployed) + 11dbac1 (Matomo-Tracker + analytics.js-Bridge + alle Events auf __matomoTrack, Custom Dimensions chapter/reached/seen). Live noch NICHT deployed — Deploy mit Nicks Freigabe. Smoke/validate/build ALL GREEN, headless gegen echten Tracker verifiziert (1 PV statt Hash-Storm, volle Event-Kette mit Dimensionen).
- 13.09.2026 (Deploy): Matomo LIVE auf info-afd.de. gh-pages 79173f7 -> 1b4863e (Rollback = 79173f7). Preview-Deploy aus /tmp/preview_build (SITE_BASE-Build, noindex verifiziert). Umami stillgelegt: Tunnel-Daemon jetzt auf CT 109 (cloudflared-Service, Token im systemd-Unit), analytics.nwk-pro.dev aus Tunnel-Ingress entfernt (404, Edge-Cache läuft nach dem Purge aus), DNS-Record bleibt (Rollback = Ingress-Regel wieder rein). CT 108 gestoppt + onboot 0, DB-Backup: /root/umami_backup_20260913.sql.gz auf PVE-Host. Matomo-Test-Hits getrimmt (frische Basis). Live-Checks: root 200 + _paq + Bridge, preview noindex, matomo UI 200.
